...
function App() {
  ...
  const [tab, setTab] = $S('home'); // home | favorites | add
  ...
  // Home density is a top-level toggle now (was only in Tweaks). We seed
  // from the tweak default so existing users keep their choice.
  ...
  const handleImport = (e) => {
    ...
    importFromFile(f, (err, recs) => {
      if (err) {
        showToast(err.message || 'שגיאה בקריאת הקובץ');
        return;
      }
      if (!recs?.length) {
  ...
  // The status bar ink follows the open detail's palette so it stays legible
  const statusInk = openRecipe ? PALETTES[openRecipe.palette].ink :
                    cookRecipe ? PALETTES[cookRecipe.palette].ink : '#2c1d27';
      ...
      <div style={{
        ...
        <Phone statusInk={statusInk}>
          ...
          <div style={{ position: 'absolute', inset: 0 }}>
            ...
            {/* Bottom nav — hide when an overlay is open */}
            {!openRecipe && !cookRecipe && !editingRecipe && (
              <BottomNav active={tab} onChange={navTo} />
            )}
          </div>
          ...
          {/* First-time hint */}
          {!hintSeen && !openRecipe && !cookRecipe && !editingRecipe && tab === 'home' && (
...
// First-time interactive hint pointing at the cards
